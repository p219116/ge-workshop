...
3. 이전 실습(실습 다. Deep Research)의 최종 결과물로 내 구글 드라이브에 저장된 **'글로벌 주요 대학의 전공 탐색 지원 체계 및 학사 지도 시스템 우수 사례 조사 보고서'**(Google Docs)를 선택해 추가합니다.
   ...
   >   만약 실습 시간 부족이나 일시적인 네트워크 장애로 인해 딥리서치 결과 보고서가 본인 드라이브에 아직 최종 생성되지 않았다면, 본 저장소의 [전공 탐색 및 학사 지도.docx](./../data/전공%20탐색%20및%20학사%20지도.docx) 링크를 클릭하여 파일 상세 페이지로 이동한 뒤, 우측 상단의 **[Download]**(또는 원본 파일 다운로드) 버튼을 클릭하여 로컬 컴퓨터에 저장합니다. 그 후 아래의 **"4. 로컬 PDF 안내서 업로드"** 가이드라인과 동일하게 이 Word(`.docx`) 파일을 소스 영역으로 끌어다 놓아 직접 업로드하여 실습을 성공적으로 진행할 수 있습니다.
