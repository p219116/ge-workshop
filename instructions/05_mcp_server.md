...
## ⚙️ 사전 필수 설정
실습을 진행하기 전에 대화창에서 **Cafeteria MCP**에 대한 인증을 사전 수행하여 정상 연동되는지 확인해야 합니다.

1. 대화창 하단 입력창 왼쪽의 커넥터 아이콘 또는 Source 아이콘을 클릭하여 **Cafeteria MCP**에 인증을 진행합니다. 인증 창이 열리면 본인의 구글 계정으로 OAuth 로그인을 수행합니다.

<img src="../images/mcp1.png" width="800">

2. 인증이 정상적으로 완료되면 아래와 같이 커넥터가 활성화된 것을 확인할 수 있습니다.
...
> [!NOTE]
> **시나리오**: 대학교 내 교직원 및 학생들의 즐거운 점심시간을 위해, 오늘의 식단 정보뿐만 아니라 이번 주 전체 식사 일정을 한눈에 실시간으로 파악하여 위트 있게 안내해 주는 **[구내 식당 메뉴 알리미]** Low-Code 에이전트를 구축합니다.

등록된 MCP는 Agent Designer에서 툴로 연계하여 강력한 개별 비서(Agent)로 커스텀할 수 있습니다. Cafeteria MCP를 도구로 사용하는 맞춤형 에이전트를 만들어 보겠습니다.  
...
<img src="../images/mcp6_v2.png" width="500">
